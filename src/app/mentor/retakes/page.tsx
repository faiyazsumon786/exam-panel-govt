...
export default function MentorRetakesPage() {
  ...
  // 1. Fetch mentor's subjects
  const { data: mentorSubjects = [] as any[] } = useQuery({
    queryKey: ['mentor-subjects-retakes'],
    queryFn: async () => {
  ...
  const mentorSubIds = mentorSubjects.map((ms: any) => ms.subject_id)

  // 2. Fetch students who are registered in the mentor's subjects
  const { data: students = [], isLoading: loadingStudents } = useQuery({
    queryKey: ['mentor-retakes-students', mentorSubIds],
    queryFn: async () => {
      ...
      // Client side filter to ensure student is enrolled in mentor's subject
      return (data || []).filter((student: any) => 
        student.student_subjects?.some((ss: any) => mentorSubIds.includes(ss.subject_id))
      )
    },
    enabled: mentorSubIds.length > 0
  })

  // 3. Fetch exams belonging to mentor's subjects
  const { data: exams = [], isLoading: loadingExams } = useQuery({
    queryKey: ['mentor-retakes-exams', mentorSubIds],
    queryFn: async () => {
      if (mentorSubIds.length === 0) return []
  ...
  const { data: attempt, isLoading: loadingAttempt, refetch: refetchAttempt } = useQuery({
    queryKey: ['mentor-student-exam-attempt', selectedStudentId, selectedExamId],
