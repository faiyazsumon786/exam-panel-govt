...
import { FileSpreadsheet, Plus, Settings, Trash2, Loader2, Calendar, Clock, Edit } from 'lucide-react'
...
export default function MentorExamsPage() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete the ${selectedExamIds.length} selected exams? All associated question copies will be deleted.`
      )
    ) {
      return
    }
  ...
  const { data: rawMentorSubjects = [], isLoading: loadingSubjects } = useQuery({
    queryKey: ['mentor-exams-subjects', profile?.id],
    queryFn: async () => {
      if (!profile) return []
      ...
      const list = data || []
      if (list.length > 0) setSubjectId((list[0] as any).subject_id)
      return list
    },
    enabled: !!profile
  })
  ...
  // 2. Fetch exams created by this mentor
  const { data: rawExams = [], isLoading: loadingExams } = useQuery({
    queryKey: ['mentor-exams', profile?.id],
    queryFn: async () => {
      if (!profile) return []
      const { data, error } = await supabase
        .from('exams')
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (res.success && res.exam) {
        ...
        handleCloseDialog()
        queryClient.invalidateQueries({ queryKey: ['mentor-exams'] })
        router.push(`/mentor/exams/${newExamId}`)
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this exam? All question copies will be deleted.')) return
    try {
      const res = await deleteExam(id)
      if (res.success) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Design examinations, assign question configurations, and publish curriculum schedules.</p>
        </div>
                  ...
                  {exams.map((exam: any) => {
                    ...
                    return (
                      <TableRow key={exam.id} className="border-b border-slate-800/60 hover:bg-slate-900/30">
                        ...
                        <TableCell className="font-semibold text-white max-w-[180px] truncate">{exam.title}</TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <Button variant="ghost" size="icon" onClick={() => router.push(`/mentor/exams/${exam.id}`)} className="h-8 w-8 text-indigo-400 hover:text-white">
